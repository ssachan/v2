...
function makeRecos()
{
	...
	foreach ( $temp as $key => &$item) {
		$dataobj = (object) doSQL(array("id"=>$item->qid,
		"SQL" => "select q.*, a.id as fid, a.firstName,a.lastName,f.bioShort,f.education from quizzes q, accounts a, faculty f where q.facultyId=a.id and f.accountId=a.id and q.available=1 and q.id = :id"
			),true);
		$dataobj->id = $item->qid;
		$dataobj->reason = $item->reason;
		$data[] = $dataobj;
