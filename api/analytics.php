...
class quizResponseDetails{
    ...
    function getQuizSummary(){
        $query = array(
            "acid"=>$this->accountId,
            "qzid"=>$this->id,
            "SQL"=>"SELECT selectedAnswers, timePerQuestion, score as testScore, data as logs, numCorrect, numIncorrect, timeTaken, maxScore FROM results WHERE accountID = :acid AND quizID = :qzid");
        $result = doSQL($query, true);
        $vars = array("selectedAnswers","timePerQuestion","testScore","numCorrect","numIncorrect","timeTaken","maxScore");
        foreach ($vars as $key => $value)
                $this->{$value} = $result->{$value};
