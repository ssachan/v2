...
window.QuizView = Backbone.View.extend({
	...
	renderQuestion : function() {
		...
		this.question.set('hasAttempted',this.hasAttempted);
	...
	handleSingleType : function(e) {
		...
		if (optionSelected == oldOptionSelected) {
			...
			renderOptions : function() {
				...
				case "1":
					...
					for ( var i = 0; i < len; i++) {
						...
						if (optionSelected != null && optionSelected == i) {
							buttonOpt.push('<button type="button" name="option" value="'
											+ i
				...
				case "2":
					...
					for ( var i = 0; i < len; i++) {
						...
						if (jQuery.inArray(i.toString(), optionSelectedArray) >= 0) {
							buttonOpt.push('<button type="button" name="mult" value="'
											+ i
											+ '" class="btn btn-large btn-block active">'
											+ String.fromCharCode(65 + i)
											+ '</button>');
				...
				case "3":
					// integer type
					htmlOpt.push('<input type="number" id="integer-type" value="'
									+ optionSelected + '">');
...
window.PracticeView = Backbone.View.extend({
	...
	renderQuestion : function() {
		...
		if (this.questionView == null) {
			this.questionView = new QuizQuestionView({el : $('#question')});
		}
		this.questionView.model = this.question;
		this.questionView.render();
