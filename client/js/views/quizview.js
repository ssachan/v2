...
window.QuizQuestionView = Backbone.View.extend({
    ...
    handleSingleType: function (e) {
        ...
        if (optionSelected == oldOptionSelected) {
            logs.add(new Log({'questionId':this.model.get('id'),'optionIndex':optionSelected,'timestamp':new Date().getTime(),'action':1}));
            ...
            logs.add(new Log({'questionId':this.model.get('id'),'optionIndex':optionSelected,'timestamp':new Date().getTime(),'action':0}));
    ...
    render: function () {
        ...
        if (this.model.get('hasAttempted')) {
            ...
            logs.add(new Log({'questionId':this.model.get('id'),'timestamp':new Date().getTime(),'action':2}));
            $('#analytics').hide();
        }
    ...
    renderInfo: function () {
        $('#solution').show();
        $('#solution').html(
            '<span class="green bold">Correct Answer</span> : ' + this.model.get('correctAnswer'));
        $('#solution').append(
            '<br><div class="sol"><span class="green bold">Solution</span> : ' + this.model.get('explanation') + '</div>');
        $('#solution')
            .append(
